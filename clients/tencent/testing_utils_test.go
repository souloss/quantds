...
func checkAPIError(t *testing.T, err error) {
	...
	// Handle API-level errors (e.g., insufficient fields, parse errors)
	errMsg := err.Error()
	if strings.Contains(errMsg, "insufficient fields") ||
		strings.Contains(errMsg, "unmarshal") ||
		strings.Contains(errMsg, "service unavailable") {
		t.Skipf("Skipping test due to API response error: %v", err)
	}
